...
class monthlyCalendar
{
    ...
    private $params = array(
        ...
        self::PARAM_CSS => true,
        self::PARAM_ORDER => self::ORDER_NONE,
        self::PARAM_LIMIT => -1
        );
    ...
    public function action() {
         ...
         * Addons which need a $_REQUEST with HTML must set this key in
    ...
    public function showCalendar()
    {
        ...
        if (($this->params[self::PARAM_SELECT_MONTH] > 0)
            ...
            $month = date('n')
                    + ($this->params[self::PARAM_SELECT_MONTH] - 100);
        } else {
            $month = date('n');
        }
        ...
        // Events einlesen
        $events = $this
                ->getEvents($month, $year, $this->params[self::PARAM_GROUP]);
        ...
        // navigation
        $navigation = array(
                'prev_link' => sprintf('%s?%s=%s&%s=%s&%s=%s',
                        $this->page_link, self::REQUEST_ACTION,
                        self::ACTION_SHOW_MONTH, self::REQUEST_MONTH,
                        $prev_month, self::REQUEST_YEAR, $prev_year),
                ...
                'next_link' => sprintf('%s?%s=%s&%s=%s&%s=%s',
                        $this->page_link, self::REQUEST_ACTION,
                        self::ACTION_SHOW_MONTH, self::REQUEST_MONTH,
                        $next_month, self::REQUEST_YEAR, $next_year));

        $head = array('0' => $this->getDayOfWeekName(0, 2, true),
        ...
        $check_today = ($this->params[self::PARAM_SHOW_TODAY]
                && (mktime(0, 0, 0, $month, 1, $year)
                        == mktime(0, 0, 0, date('n'), 1, date('Y')))) ? true
                : false;
        ...
        while ($i < 50) {
            ...
            if (!$complete) {
                if (in_array($i, $events)) {
                    ...
                    $week[$dow]['date'] = $i;
                    $week[$dow]['link'] = sprintf(
                            '%s?%s=%s&%s=%s&%s=%s&%s=%s', $this->response_link,
                            self::REQUEST_EVENT, self::EVENT_DAY,
                            self::REQUEST_MONTH, $month, self::REQUEST_DAY, $i,
                            self::REQUEST_YEAR, $year);
        ...
        $data = array(
                ...
                'show_navigation' => ($this->params[self::PARAM_NAVIGATION]) ? 1
                        : 0,
                'link_month' => (int) $this->params[self::PARAM_LINK_MONTH],
                'navigation' => $navigation, 'head' => $head, 'month' => $mon);
