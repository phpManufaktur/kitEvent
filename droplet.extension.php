...
function kit_event_droplet_header($page_id) {
	...
	if ((isset($_REQUEST[eventFrontend::request_action]) && ($_REQUEST[eventFrontend::request_action] == eventFrontend::action_event)) &&
		...
		if (count($event) > 0) {
			$event = $event[0];
			$result = array(
				'title'				=> isset($event[dbEventItem::field_title]) ? $event[dbEventItem::field_title] : '',
				'description'	=> isset($event[dbEventItem::field_desc_short]) ? $event[dbEventItem::field_desc_short] : '',
